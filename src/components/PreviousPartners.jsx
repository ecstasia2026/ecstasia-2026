import React from "react";
import Marquee from "react-fast-marquee";

function PreviousPartners() {
  return (
    <div className="w-full min-h-[40vh] sm:min-h-[50vh] bg-[#20110B] flex flex-col justify-center">
      <h1 className="text-white font-heading text-center py-8 sm:py-12 text-3xl sm:text-5xl lg:text-7xl">
        Our Previous Partners
      </h1>

      <Marquee speed={60} pauseOnHover gradient={false}>
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770787641/xoppfvqt877f0dwtuabj.png"
          alt="Decathlon Logo"
          className="h-15 sm:h-10 lg:h-10 mx-6 sm:mx-10 object-contain"
        />
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770921385/gowtoeemlbjc8bieaf4v.png"
          alt="91.9 friends"
          className="h-15 sm:h-10 lg:h-20 mx-6 sm:mx-10 object-contain"
        />
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770920388/i7hrpr5d6xhiq7h8qvct.png"
          alt="Decathlon Logo"
          className="h-15 sm:h-10 lg:h-20 mx-6 sm:mx-10 object-contain"
        />
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770920405/qdtxyfdqhnw4x80wojpv.png"
          alt="Decathlon Logo"
          className="h-15 sm:h-10 lg:h-20 mx-6 sm:mx-10 object-contain"
        />
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770920481/xz7tlykvencybtkueixg.png"
          alt="Decathlon Logo"
          className="h-15 sm:h-10 lg:h-20 mx-6 sm:mx-10 object-contain"
        />
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770920512/s5j7em8swoyb8mktt8ng.png"
          alt="Decathlon Logo"
          className="h-15 sm:h-10 lg:h-20 mx-6 sm:mx-10 object-contain"
        />
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770920612/p44kjijpefkquyqdqhu2.png"
          alt="Decathlon Logo"
          className="h-15 sm:h-10 lg:h-20 mx-6 sm:mx-10 object-contain"
        />
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770920637/joncddvby0x62aynqm9h.png"
          alt="Decathlon Logo"
          className="h-15 sm:h-10 lg:h-20 mx-6 sm:mx-10 object-contain"
        />
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770920660/nvk51zbqi6mifvsv4slt.png"
          alt="Decathlon Logo"
          className="h-15 sm:h-10 lg:h-20 mx-6 sm:mx-10 object-contain"
        />
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770920681/np5hg5mgssx3b3lyt6pz.png"
          alt="Decathlon Logo"
          className="h-15 sm:h-10 lg:h-20 mx-6 sm:mx-10 object-contain"
        />
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770920702/of3p1tlskocuijxfa6ui.png"
          alt="Decathlon Logo"
          className="h-15 sm:h-10 lg:h-20 mx-6 sm:mx-10 object-contain"
        />
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770920721/xfhgv5rxsclnyb3aag7b.png"
          alt="Decathlon Logo"
          className="h-15 sm:h-10 lg:h-20 mx-6 sm:mx-10 object-contain"
        />
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770920750/an5l3euaoaz290pwt0jd.png"
          alt="Decathlon Logo"
          className="h-15 sm:h-10 lg:h-20 mx-6 sm:mx-10 object-contain"
        />
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770920772/dnmvkdotwuocketeukva.jpg"
          alt="Decathlon Logo"
          className="h-15 sm:h-10 lg:h-20 mx-6 sm:mx-10 object-contain"
        />
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770920797/lh2qo9u7znxmvvc8c82k.png"
          alt="Decathlon Logo"
          className="h-15 sm:h-10 lg:h-20 mx-6 sm:mx-10 object-contain"
        />
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770920815/zrqxnafbolhzai7qwguy.png"
          alt="Decathlon Logo"
          className="h-15 sm:h-10 lg:h-20 mx-6 sm:mx-10 object-contain"
        />
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770920836/oot7zp6vydh4zuzjtvx6.png"
          alt="Decathlon Logo"
          className="h-15 sm:h-10 lg:h-20 mx-6 sm:mx-10 object-contain"
        />
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770920860/m90tkpaadmelh5u0vjcj.png"
          alt="Decathlon Logo"
          className="h-15 sm:h-10 lg:h-20 mx-6 sm:mx-10 object-contain"
        />
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770920887/qykhv72eyvl3e5lee861.png"
          alt="Decathlon Logo"
          className="h-15 sm:h-10 lg:h-20 mx-6 sm:mx-10 object-contain"
        />
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770920906/sxljpnarada6mupso6vu.png"
          alt="Decathlon Logo"
          className="h-15 sm:h-10 lg:h-20 mx-6 sm:mx-10 object-contain"
        />
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770920924/sd4isuvkoznpn6vucxyx.png"
          alt="Decathlon Logo"
          className="h-15 sm:h-10 lg:h-20 mx-6 sm:mx-10 object-contain"
        />
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770920965/nvna5ugfgym85nq5tmt8.png"
          alt="Decathlon Logo"
          className="h-15 sm:h-10 lg:h-20 mx-6 sm:mx-10 object-contain"
        />
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770920985/goqp4k2wui4alevbbal6.png"
          alt="Decathlon Logo"
          className="h-15 sm:h-10 lg:h-20 mx-6 sm:mx-10 object-contain"
        />
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770921011/cvjcvrvkltpl7pvzkgmt.png"
          alt="Decathlon Logo"
          className="h-15 sm:h-10 lg:h-20 mx-6 sm:mx-10 object-contain"
        />
        <img
          src="https://res.cloudinary.com/dooekcvv0/image/upload/v1770921042/ewyhm3w0kflr6zohlj7a.png"
          alt="Decathlon Logo"
          className="h-15 sm:h-10 lg:h-20 mx-6 sm:mx-10 object-contain"
        />
      </Marquee>
    </div>
  );
}

export default PreviousPartners;
